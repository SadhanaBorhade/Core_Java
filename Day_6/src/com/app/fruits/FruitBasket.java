	...
	public static void main(String[] args) {
		...
		while(!exit)
		{
			...
			case 7:
				...
				break;
			case 8:
				System.out.println("Enter the index of fruit to display its functionality:");
				int index1=sc.nextInt();
				if(index1<f.length)
				{
					if(f[index1] instanceof Mango)
					{
						((Mango) f[index1]).pulp();
					}
					else if(f[index1] instanceof Orange)
					{
						((Orange) f[index1]).juice();
					}
					else if(f[index1] instanceof Apple)
					{
						((Apple) f[index1]).jam();
					}
				}
